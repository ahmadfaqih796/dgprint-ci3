...
class BayarController extends CI_Controller
{
	...
	public function bayar($id)
	{
		...
		$data = array(
			...
			'title' => 'Pembayaran | Anugrah Sejahtera'
		);
		$this->load->view('frontend/templates/header', $data);
	...
	{
		$dataBank = array(
			'bni' => 'Bank BNI 123456789 Atas Anugrah Sejahtera ',
			'bri' => 'Bank BRI 115511026 Atas Anugrah Sejahtera '
		);
		$data = array(
			'bank' => $dataBank[$bank],
			'pesanan' => $this->BayarModel->lihat_keranjang_status_selesai($this->session->userdata('session_id'), 'bayar_menunggu', $id)->row_array(),
			'title' => 'Terima Kasih | Anugrah Sejahtera'
		);
		$this->load->view('frontend/templates/header', $data);
	...
	public function konfirmasi($id)
	{
		if (isset($_POST['konfirmasi'])) {
			...
			$data = array(
				'title' => 'Konfirmasi Pembayaran | Anugrah Sejahtera',
